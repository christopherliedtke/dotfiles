...
# Note: blocked by SIP on Apple Silicon Macs
sudo nvram SystemAudioVolume=" " 2>/dev/null

# Disable transparency in the menu bar and elsewhere
# Note: com.apple.universalaccess requires Full Disk Access on modern macOS
...
# Note: removed in macOS 11+, Notification Center is now part of Control Center
...
# Keep automatic period substitution enabled
# defaults write NSGlobalDomain NSAutomaticPeriodSubstitutionEnabled -bool false

# Disable smart quotes as they’re annoying when typing code
defaults write NSGlobalDomain NSAutomaticQuoteSubstitutionEnabled -bool false

# Keep auto-correct enabled
...
# Keyboard repeat rate (keep macOS defaults)
...
# Finder icon view settings via PlistBuddy
# Note: plist may not exist on a fresh install — skip errors silently
FINDER_PLIST=~/Library/Preferences/com.apple.finder.plist
if [ -f "$FINDER_PLIST" ]; then
    # Show item info near icons on the desktop and in other icon views
    /usr/libexec/PlistBuddy -c "Set :DesktopViewSettings:IconViewSettings:showItemInfo true" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :FK_StandardViewSettings:IconViewSettings:showItemInfo true" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :StandardViewSettings:IconViewSettings:showItemInfo true" "$FINDER_PLIST" 2>/dev/null

    # Show item info to the right of the icons on the desktop
    /usr/libexec/PlistBuddy -c "Set DesktopViewSettings:IconViewSettings:labelOnBottom false" "$FINDER_PLIST" 2>/dev/null

    # Enable snap-to-grid for icons on the desktop and in other icon views
    /usr/libexec/PlistBuddy -c "Set :DesktopViewSettings:IconViewSettings:arrangeBy grid" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :FK_StandardViewSettings:IconViewSettings:arrangeBy grid" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :StandardViewSettings:IconViewSettings:arrangeBy grid" "$FINDER_PLIST" 2>/dev/null

    # Increase grid spacing for icons on the desktop and in other icon views
    /usr/libexec/PlistBuddy -c "Set :DesktopViewSettings:IconViewSettings:gridSpacing 100" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :FK_StandardViewSettings:IconViewSettings:gridSpacing 100" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :StandardViewSettings:IconViewSettings:gridSpacing 100" "$FINDER_PLIST" 2>/dev/null

    # Increase the size of icons on the desktop and in other icon views
    /usr/libexec/PlistBuddy -c "Set :DesktopViewSettings:IconViewSettings:iconSize 80" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :FK_StandardViewSettings:IconViewSettings:iconSize 80" "$FINDER_PLIST" 2>/dev/null
    /usr/libexec/PlistBuddy -c "Set :StandardViewSettings:IconViewSettings:iconSize 80" "$FINDER_PLIST" 2>/dev/null
fi
...
# Remove Dropbox’s green checkmark icons in Finder (skip if Dropbox not installed)
...
# 13: Lock Screen
# Hot corners — all disabled (no-op)
...
# Note: may be blocked by SIP on modern macOS
